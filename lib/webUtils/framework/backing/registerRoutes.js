...
const sanitize = (obj, logger) => {
  ...
  const subset = Object.fromEntries(Object.entries(obj)
    .filter(([key]) => Object.keys(routePattern).includes(key)));
...
const registerRoutes = (
  routesToBeRegistered, framework, logger,
) => {
  ...
  routes.forEach((routeToBeRegistered) => {
    ...
    route.handlers.forEach(({ method, middleware }) => (
      ...
      (method === 'ws')
        ? mountRouter[method](route.path[0],
