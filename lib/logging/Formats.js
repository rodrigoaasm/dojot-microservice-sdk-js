...
const textFormat = winston.format.combine(
  ...
  winston.format.printf((info) => {
    ...
    // text to be logged
    const text = `${info.timestamp} | sid=${info.sid} | level=${info.level} | ${(info.file && info.line) ? `file=(${info.file}:${info.line}) | ` : ''}message=${info.message}${info.rid ? ` | rid=${info.rid}` : ''}${(stringifiedExtra !== '{}') ? ` | extra=${stringifiedExtra}` : ''}`;
    return text;
  }),
);
