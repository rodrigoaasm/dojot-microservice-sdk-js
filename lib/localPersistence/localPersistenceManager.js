...
class LocalPersistenceManager {
   ...
   */
  constructor(logger, readInMemory = true, databasePath = DEFAULT_DATABASE_PATH) {
    this.logger = logger;
  ...
  async init() {
    ...
    try {
      ...
      this.managementLevel = subleveldown(this.db, 'managementLevel', { keyEncoding: 'utf8', valueEncoding: 'json' });
  ...
  async copyLevelsToMemory() {
    ...
    const managementLevelWritableStream = Writable({
      async write(key, encoding, cb) {
        ...
        const sublevel = subleveldown(tmpDB, key.toString(), sublevelOptions);
  ...
  async initializeLevel(sublevel, options = DEFAULT_GET_OPTIONS) {
    ...
    try {
      ...
      const diskSubLevel = subleveldown(this.db, sublevel, tmpOptions);
      ...
      const diskSublevel = subleveldown(this.db, sublevel, options);
  ...
  */
  async executeBatchForLevels(operationsForLevel) {
    ...
    for (const [key, value] of operationsForLevel) {
      ...
      try {
        const [diskSublevel, memorySubLevel] = await this.initializeLevel(levelOperations.config.name, levelOperations.config.options,);
  ...
  async clear(sublevelName) {
    const [sublevelDisk, sublevelMemory] = await this.initializeLevel(sublevelName);
