...
class Producer {
  ...
  */
  constructor(config) {
    ...
    this.isReady = false;
    this.producer = new Kafka.Producer(this.config['kafka.producer'],
  ...
  */
   ...
   */
  produce(
    topic, message, key = null, partition = null,
  ) {
    if (this.isReady === false) {
      logger.debug('It is not yet ready to produce.');
      return Promise.reject(new Error('It is not yet ready to produce.'));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.producer.produce(
        topic, partition, buffer, key, timestamp, { callback },
      );
   ...
   */
  async getStatus() {
    return Helper.getStatus(this.producer);
  }
