...
module.exports = class CommitManager {
  ...
  async finish() {
    ...
    logger.debug('CommitManager finished successfully.. committing processed offsets');
    return this.commitProcessedOffsets();
  }
  ...
  notifyFinishedProcessing(data) {
    ...
    const record = partitionOffsets.filter((currRecord) => currRecord.offset === offset)[0];
    if (record) {
      record.done = true;
    }
  }
