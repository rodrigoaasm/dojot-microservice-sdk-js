...
module.exports = class Consumer {
  ...
  unregisterCallback(registerId) {
    ...
    if (topicsRegExpBefore !== this.topicRegExpArray.length) {
      ...
      Object.keys(this.topicMap).forEach((topic) => {
        this.topicMap[topic] = this.topicMap[topic].filter((entry) => entry.id !== registerId);
  ...
  backoffWithRandomDelta(retries) {
    const waitingTime = Math.min(this.config['subscription.backoff.max.ms'],
    ...
    if (!this.isWaitingForRefreshSubscriptions && this.isReady) {
      ...
      const subscriptionProcedure = (retries = 0) => {
        ...
        try {
          ...
          logger.debug(`subscribing in the following topics (${
            topics.length
          ...
          setTimeout(
            subscriptionProcedure, timeout, retries + 1,
          );
        }
      };
        ...
        if (entry.regExp.test(data.topic)) {
          ...
          do {
            ...
            try {
              ...
              logger.warn(`Error on user's callback ${entry.id} topic: ${
                data.topic
          ...
          if (!succeeded) {
            ...
            this.eventEmitter.emit(
              'error.processing', entry.id, data,
            );
          }
        }
      });
